...
define( 'UNICO_ROOT', get_template_directory() . '/' );
...
add_action( 'after_setup_theme', 'unico_wp_load', 5 );

function unico_wp_load() {

	defined( 'UNICO_URL' ) or define( 'UNICO_URL', get_template_directory_uri() . '/' );
	define(  'UNICO_KEY','!@#unico');
	define(  'UNICO_URI', get_template_directory_uri() . '/');

	if ( ! defined( 'UNICO_NONCE' ) ) {
		define( 'UNICO_NONCE', 'unico_wp_theme' );
	}

	( new \UNICO\Includes\Classes\Base )->loadDefaults();
	( new \UNICO\Includes\Classes\Ajax )->actions();

}
add_action( 'init', 'unico_bunch_theme_init');
function unico_bunch_theme_init()
{
