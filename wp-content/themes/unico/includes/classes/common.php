...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => unico_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => unico_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => unico_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => unico_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => unico_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => unico_meta( 'banner_elementor_template', $page_id ),
				'banner'            => unico_set( unico_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => unico_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => unico_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : unico_the_title( $template ),
				'banner'        => unico_set( $bg, 'url' ),
