...
class Header_Enqueue {
	...
	public static function scripts() {
		$options = get_theme_mod( 'unico' . '_options-mods' );
		$ssl     = is_ssl() ? 'https' : 'http';

		$scripts = array(
		);

		$scripts = apply_filters( 'UNICO/includes/classes/header_enqueue/scripts', $scripts );
		/**
		 * Enqueue the scripts
		 *
		 * @var array
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( UNICO_NONCE ),
		);

		wp_localize_script( 'jquery', 'unico_data', $header_data );

		if ( unico_set( $options, 'footer_js' ) ) {

			wp_add_inline_script( 'jquery', unico_set( $options, 'footer_js' ) );
		}
	...
	public static function styles() {
		...
		$styles = apply_filters( 'UNICO/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				wp_enqueue_style( "unico-{$name}", $style );
			} else {
				wp_enqueue_style( "unico-{$name}", get_template_directory_uri() . '/' . $style );
			}
		}
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            $header_styles .= unico_custom_fonts_load( $custom_font );
        }

		wp_add_inline_style( 'unico-main-style', $header_styles );
	}
	...
	public static function resource_hints( $urls, $relation_type ) {
		if ( wp_style_is( 'unico-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
			$urls[] = array(
	...
	public static function header_styles() {
		...
		$data = \UNICO\Includes\Classes\Common::instance()->data( 'blog' )->get();
		...
		$settings = get_theme_mod( 'unico' . '_options-mods' );

		if ( $custom_font = unico_set( $settings, 'theme_custom_font' ) ) {

			$styles .= apply_filters('unico_redux_custom_fonts_load', $custom_font );
