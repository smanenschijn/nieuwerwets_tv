...
class Visual_Composer {
	...
	function init() {
		...
		$maps = apply_filters( 'unico_vc_map', $this->maps );
		...
		foreach ( $maps as $value) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( function_exists( 'wpunico_shortcode' ) ) {
					$tag = esc_attr( unico_set( $data, 'base' ) );

					wpunico_shortcode( $tag, array( $this, 'output' ) );
				}
			}
		}
	}
	...
	function output( $atts, $content = null, $tag ) {
		...
		unico_template_load( 'shortcodes/' . $tag . '.php', compact( 'atts', 'content', 'tag' ) );
	...
	function get_file( $tag ) {
		...
		$file = unico_template( 'includes/resource/vc_map/'.$tag . '.php' );
		$file = apply_filters( "unico_vc_map_file_{$tag}", $file );
	...
	function toggle( $settings, $value ) {
		...
		$values = isset( $settings['value'] ) && is_array( $settings['value'] ) ? $settings['value'] : array( esc_html__( 'Yes', 'unico' ) => 'true' );
		if ( ! empty( $values ) ) {
