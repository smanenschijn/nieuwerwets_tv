 ...
 * @since 1.0.0
 */
class Seo_Report extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Seo Report', 'unico' );
	} 
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'seo_report',
			[
				'label' => esc_html__( 'Seo Report', 'unico' ),
			]
		);
		$this->add_control(
			'bg_image',
		...
		$this->add_control(
            'style_two',
            [
				'label'   => esc_html__( 'Choose Enable/Disable Spacing Style', 'unico' ),
				'label_block' => true,
				'type'    => Controls_Manager::SELECT,
				'default' => 'one',
				'options' => array(
					'one' => esc_html__( 'Enable Bottom Space', 'unico' ),
					'two'  => esc_html__( 'Disable Bottom Space', 'unico' )
				),
			]
        );
	...
	protected function render() {
        ...
        <section class="image-bg default-bg" <?php if(($settings['bg_color']) || ($settings['bg_image']['id'])): ?>style="background:<?php echo wp_kses( $settings['bg_color'], true );?> <?php if($settings['bg_image']['id']): ?>url(<?php echo esc_url(wp_get_attachment_url($settings['bg_image']['id'])); ?>) repeat;"<?php endif; ?><?php endif; ?>>
            <div class="container">
                ...
                <?php echo do_shortcode( $settings['seo_report_form'] );?>
                
            </div>
            <?php if($settings['style_two'] == 'two') echo ''; else echo '<div class="ht-80"></div>'; ?>
        </section>
